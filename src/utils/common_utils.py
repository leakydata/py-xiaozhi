"""
Common utility function collection module. Contains common utility functions such as text-to-speech, browser operations, and clipboard.
"""
...
def _warm_up_audio_device():
    """
    Warm up the audio device to prevent the first word from being swallowed.
    """
...
def _audio_queue_worker():
    """
    Audio queue worker thread to ensure audio is played in order and not truncated.
    """
    ...
    while _audio_worker_running:
        try:
            ...
            with _audio_lock:
                ...
                time.sleep(0.5)  # Pause after playback to prevent the tail from being swallowed
...
def open_url(url: str) -> bool:
    try:
        ...
        if success:
            ...
            logger.warning(f"Failed to open webpage: {url}")
        return success
    except Exception as e:
...
def extract_verification_code(text: str) -> Optional[str]:
    try:
        ...
        # Activation related keyword list
        activation_keywords = [
            "login",
            "control panel",
            "activate",
        ...
        # Match 6-digit verification code, possibly separated by spaces
        patterns = [
        ...
        # If there are activation keywords but no precise pattern is matched, try the original pattern
        ...
        if match:
            ...
            if len(code) == 6 and code.isdigit():
                logger.info(f"Verification code extracted from text (generic pattern): {code}")
                return code
